...
namespace Services.Services
{
    public class TestTechService : BaseService, ITestTechService
    {
        public async Task<ResponseResult> AddStationDataAsync(AddStationDataViewModel model, string token)
        {
            ...
            using (var response = await httpClient.PostAsync("api/TestTech/AddStationData", content))
            {
        ...
        public async Task<List<VTestResult>> GetFinalResultAsync(int wcId, string token)
        {
            ...
            using (var response = await httpClient.GetAsync("api/TestTech/getfinalresult/" + wcId))
            {
                string apiResponse = await response.Content.ReadAsStringAsync();
