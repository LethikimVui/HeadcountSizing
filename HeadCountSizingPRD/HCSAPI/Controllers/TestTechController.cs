...
namespace HCSAPI.Controllers
{
    ...
    public class TestTechController : ControllerBase
    {
        ...
        {
            var results = await context.Query<VStation>().AsNoTracking().FromSql(SPTestTech.GetStationList, wcId).ToListAsync();
            return results;
        }
        [HttpPost("getquantity")]
        public async Task<List<VEquipmentQuantity>> getquantity([FromBody] GetEquipmentQuantity model)
        {
            var results = await context.Query<VEquipmentQuantity>().AsNoTracking().FromSql(SPTestTech.Equipment_get_quantity_by_station, model.StationOrTester, model.CustId).ToListAsync();
            return results;
        }
        ...
        public async Task<IActionResult> AddStationData([FromBody] AddStationDataViewModel model)
        {
            try
            {
                await context.Database.ExecuteSqlCommandAsync(SPTestTech.Equipment_add, model.CustId, model.MonthId, model.Station, model.Quantity, model.CreatedBy);
        ...
        public async Task<IActionResult> UpdateStationQuantity([FromBody] UpdateStationQuantityViewModel model)
        {
            try
            {
                await context.Database.ExecuteSqlCommandAsync(SPTestTech.Equipment_update, model.CustId, model.MonthId, model.StationOrTester, model.Quantity, model.UpdatedBy);
                return Ok(new ResponseResult(200));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> UpdateDowntime([FromBody] UpdateDowntimeViewModel model)
        {
            try
            {
                await context.Database.ExecuteSqlCommandAsync(SPTestTech.Activities_update, model.CustId, model.DowntimeType, model.DowntimeDetail, model.StationOrTester, model.Duration, model.UpdatedBy);
                return Ok(new ResponseResult(200));
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> UpdateTestTech([FromBody] UpdateLockedHeadcountViewModel model)
        {
            try
            {
                await context.Database.ExecuteSqlCommandAsync(SPTestTech.CurrentHC_update, model.CustId, model.Sep, model.Oct, model.Nov, model.Dec, model.Jan, model.Feb, model.Mar, model.Apr, model.May, model.Jun, model.Jul, model.Aug, model.UpdatedBy);
                return Ok(new ResponseResult(200));
            }
            catch (Exception ex)
            {
